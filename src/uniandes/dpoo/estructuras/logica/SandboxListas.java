...
public class SandboxListas
{
    ...
    {
    	Object[] array = listaEnteros.toArray();
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	List<Integer> lista = IntStream.of(nuevosEnteros).boxed().collect(Collectors.toList());
    	listaEnteros = lista;
    }
    ...
    {	
    	...
    	for(int i = 0; i < listaEnteros.size(); i++) {
    		if(mapa.containsKey(listaEnteros.get(i))) {
    			mapa.put(listaEnteros.get(i), mapa.get(listaEnteros.get(i)) + 1);
    		}
    		else {
    			mapa.put(listaEnteros.get(i), 1);
    		}
    	}
    	int counter = 0;
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
    	...
    	for(int i = 0; i < cantidad; i++) {
    		lista.add(minimo + (int) (Math.random() * (maximo - minimo + 1)));
    	}
    	listaEnteros = lista;
    }
